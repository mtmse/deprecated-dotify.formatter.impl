...
public class PageImpl implements Page {
    ...
    private float staticAreaSpaceNeeded() {
        return rowsNeeded(
            pageAreaTemplate.getBefore(),
            master.getRowSpacing()
        ) + rowsNeeded(
            pageAreaTemplate.getAfter(),
            master.getRowSpacing()
        );
    }
    ...
    /**
     * Adds the footer area if not already added.
     * Note that this area also contains the page area if aligned to be the bottom.
     */
    ...
    private boolean hasBodyRowsLeft() {
        ...
        return
            Math.ceil(finalRows.getOffsetHeight() + areaSize) <
