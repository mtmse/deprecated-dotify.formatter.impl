...
public class BlockContentManager extends AbstractBlockContentManager {
	 ...
	 */
	private boolean ensureBuffer(int index, boolean wholeWordsOnly, int spareWidth) {
		while (index<0 || rows.size()<index) {
			if (!sp.hasMoreData()) {
				return false;
			}
			sp.getNext(wholeWordsOnly, spareWidth).ifPresent(v->rows.add(v));
		}
	...
	public boolean hasNext() {
		...
		if (diff==0) {
			if (!sp.hasMoreData()) {
				...
				return new SegmentProcessor(sp).getNext(false, 0).isPresent();
			}
		} else if (diff<0) {
	...
	public Optional<RowImpl> getNext(boolean wholeWordsOnly, int spareWidth) {
		if (ensureBuffer(rowIndex+1, wholeWordsOnly, spareWidth)) {
